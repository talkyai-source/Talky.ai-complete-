...
type CampaignFormDraft = {
     ...
     * lead-field save) can retry against the same row instead of creating a
     * duplicate. Persisting it here means that retry still works after a
     * real navigation away and back, not just in-place.
     */
    createdCampaignId: string | null;
};

function readFormDraft(key: string | undefined): Partial<CampaignFormDraft> | null {
    if (!key || typeof window === "undefined") return null;
    try {
        const raw = window.sessionStorage.getItem(key);
        if (!raw) return null;
        const parsed: unknown = JSON.parse(raw);
        return parsed && typeof parsed === "object" ? (parsed as Partial<CampaignFormDraft>) : null;
    } catch {
        return null;
    }
}

function writeFormDraft(key: string | undefined, draft: CampaignFormDraft): void {
...
interface Props {
    ...
    /**
     * When set, the fields below are mirrored to sessionStorage under this
     * key so a real navigation away and back (not just an in-place retry)
     * does not lose them. Owned and generated by the caller — see
     * `app/inbound-campaigns/new/page.tsx`. Omitted for outbound creation and
     * for edit mode, which keep today's non-persisted behaviour.
     */
...
export function CampaignForm({ mode, campaignId, initialData, afterCreateHref, direction = "outbound", draftStorageKey }: Props) {
    const router = useRouter();
    const isEdit = mode === "edit";
    const seed = initialData ?? EMPTY_INITIAL;

    // Read once on mount; re-renders never re-read this, only re-write it.
    // Never applies in edit mode — edit always seeds from `initialData`.
    const [restoredDraft] = useState<Partial<CampaignFormDraft> | null>(() => (isEdit ? null : readFormDraft(draftStorageKey)));
    ...
    const [formData, setFormData] = useState({
        name: typeof restoredDraft?.name === "string" ? restoredDraft.name : seed.name,
        description: typeof restoredDraft?.description === "string" ? restoredDraft.description : seed.description,
        system_prompt: typeof restoredDraft?.system_prompt === "string" ? restoredDraft.system_prompt : seed.system_prompt,
        voice_id: typeof restoredDraft?.voice_id === "string" ? restoredDraft.voice_id : seed.voice_id,
        goal: typeof restoredDraft?.goal === "string" ? restoredDraft.goal : seed.goal,
    });

    const [personaType, setPersonaType] = useState<PersonaType>(restoredDraft?.personaType ?? seed.persona_type);
    const [companyName, setCompanyName] = useState<string>(typeof restoredDraft?.companyName === "string" ? restoredDraft.companyName : seed.company_name);
    const [agentNamesRaw, setAgentNamesRaw] = useState<string>(typeof restoredDraft?.agentNamesRaw === "string" ? restoredDraft.agentNamesRaw : seed.agent_names.join(", "));
    ...
    const [slotValues, setSlotValues] = useState<Record<string, string>>(
        restoredDraft?.slotValues && typeof restoredDraft.slotValues === "object" ? restoredDraft.slotValues : { ...seed.slots },
    );
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        ...
                        <textarea
                            ...
                            disabled={submitting}
                            rows={21}
                            className="flex w-full min-h-[600px] resize-y rounded-lg border border-input bg-background px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-not-allowed disabled:opacity-50"
                        />
