...
|---|---|---|---|---|---|
| 0 | Git-based prod deploy + production hygiene | 🔴 P0 (added 2026-06-02) | **DONE** | `ca06117` `99f0afa` `bcb655b` (2026-06-02) | Root cause of repeated git↔prod drift: prod was a plain rsync-from-laptop copy (`.git` excluded), so deletes never propagated and in-place edits never reached git. Converted `/opt/talky` to a **git checkout tracking origin/main** with sparse-checkout (backend + telephony + gateway; frontends excluded → Vercel). Removed from prod: Talk-Leee/, Admin/, dead shadowed `auth.py`, `_backup_pre_telephony_split/`, `.deploy-backups/`, 40 `.bak` files, root `.md`/stray rot. Repo cleaned of generated status-report rot. **Caught + fixed a P0 in the process:** item-9's `ruff --fix` F401 pass had stripped re-exported symbols (get_user_tenants, hash_session_token, PERSONAS, …), which broke `import app.main` — only surfaced when prod pulled git. Reverted that autofix (kept the 3 F821 fixes + item-2). `deploy_to_server.sh` is now git-pull-based. All 5 services healthy on git HEAD. |
...
| 2 | Decompose `voice_pipeline_service.py` + `telephony_bridge.py` + `voice_orchestrator.py` | 🔴 P0 | **IN PROGRESS** | `51a473e` `4a8fad9` (2026-06-02) | telephony_bridge `make_call` decomposed: caller-ID guard → `caller_id_guard.py`, pre-warm pipeline → `prewarm.py`. make_call 535→323 lines, file 1054→842. Deployed + soaking. **Remaining:** finish telephony_bridge (CallGuard decision handling, originate/store), then `voice_pipeline_service.py` + `voice_orchestrator.py`. |
