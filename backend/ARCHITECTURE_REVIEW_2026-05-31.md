...
|---|---|---|---|---|---|
| 1 | Move telephony module state to Redis | 🔴 P0 | code complete (5/5 steps) — pending prod soak | `aacc6cc` `64cc742` `c1e170b` `4bcb96a` `f3e3b74` (2026-05-31) | Phase 1 plan: `~/.claude/plans/alight-lets-plan-the-happy-pearl.md`. **Steps 1-5 done & committed, default flag `memory` → zero behaviour change until flipped.** Step 1 abstraction; step 2 consumers migrated; step 3 RedisBackedStateBackend write-through mirror; step 4 per-process-incarnation heartbeat + dead-process orphan recovery (safe under `--workers N`); step 5 debounced TTL refresh on the audio path. Remaining: deploy code (no-op at `memory`), flip `TELEPHONY_STATE_BACKEND=redis` in a maintenance window, run the live restart-mid-call verification (see plan §Verification), soak 3-5 days, then mark ✅. Phase 2 (multi-pod active-active) is a separate future plan. |
...
| 3 | Replace HTTP-to-self in dialer worker | 🔴 P0 | not started | — | direct service-class call instead of POST |
| 4 | Pick one migration system | 🟠 P1 | not started | — | recommend Alembic; backfill raw SQL into versions |
...
| 9 | CI lint (ruff + mypy --strict) | 🟡 P2 | not started | — | block PRs on regressions |
