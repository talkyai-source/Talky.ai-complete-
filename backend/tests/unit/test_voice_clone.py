...
async def test_count_for_tenant_filters_by_tenant():
    ...
    n = await vcs.count_for_tenant(_FakePool(conn), TENANT_ID)
    assert n == 3
    sql, args = conn.calls[0]
    assert "WHERE tenant_id = $1" in sql and args == (TENANT_ID,)


@pytest.mark.asyncio
async def test_owned_voice_ids_set():
    conn = _FakeConn(fetch=[{"voice_id": "v1"}, {"voice_id": "v2"}])
    ids = await vcs.owned_voice_ids(_FakePool(conn), TENANT_ID)
    assert ids == {"v1", "v2"}
    assert conn.calls[0][1] == (TENANT_ID,)
...
async def test_record_clone_returns_dict():
    ...
    out = await vcs.record_clone(
        _FakePool(conn), tenant_id=TENANT_ID, voice_id="vX", name="My voice",
...
async def test_get_owned_is_tenant_scoped():
    ...
    out = await vcs.get_owned(_FakePool(conn), TENANT_ID, "clone-9")
    assert out is None
    sql, args = conn.calls[0]
    assert "tenant_id = $1 AND id = $2" in sql and args == (TENANT_ID, "clone-9")


@pytest.mark.asyncio
async def test_delete_owned_reports_rowcount():
    conn = _FakeConn(execute="DELETE 1")
    assert await vcs.delete_owned(_FakePool(conn), TENANT_ID, "c1") is True
    conn2 = _FakeConn(execute="DELETE 0")
    assert await vcs.delete_owned(_FakePool(conn2), TENANT_ID, "c1") is False
