...
CREATE TABLE public.abuse_events (
    ...
    CONSTRAINT abuse_events_severity_check CHECK (((severity)::text = ANY ((ARRAY['low'::character varying, 'medium'::character varying, 'high'::character varying, 'critical'::character varying])::text[])))
);
...
CREATE TABLE public.suspension_events (
    ...
    audit_log_id uuid,
    CONSTRAINT suspension_events_target_type_check CHECK (((target_type)::text = ANY ((ARRAY['user'::character varying, 'tenant'::character varying, 'partner'::character varying])::text[])))
);
...
CREATE TABLE public.call_guard_decisions (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT call_guard_decisions_decision_check CHECK (((decision)::text = ANY ((ARRAY['allow'::character varying, 'block'::character varying, 'queue'::character varying, 'throttle'::character varying])::text[])))
);
...
CREATE TABLE public.emergency_access_requests (
    ...
    reviewed_by uuid,
    review_notes text,
    CONSTRAINT emergency_access_requests_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'approved'::character varying, 'denied'::character varying, 'expired'::character varying, 'used'::character varying, 'cancelled'::character varying])::text[])))
);
...
CREATE VIEW public.expiring_secrets AS
 SELECT secret_id,
  ...
  WHERE ((is_active = true) AND (expires_at IS NOT NULL) AND (expires_at < (now() + '7 days'::interval)));
...
CREATE TABLE public.security_events (
    ...
    CONSTRAINT security_events_status_check CHECK (((status)::text = ANY ((ARRAY['open'::character varying, 'investigating'::character varying, 'resolved'::character varying, 'false_positive'::character varying, 'escalated'::character varying])::text[])))
);
...
CREATE VIEW public.open_security_events AS
 SELECT event_id,
  ...
  WHERE ((status)::text = ANY ((ARRAY['open'::character varying, 'investigating'::character varying, 'escalated'::character varying])::text[]));
...
CREATE TABLE public.recordings_s3 (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT recordings_s3_status_check CHECK (((status)::text = ANY ((ARRAY['uploading'::character varying, 'uploaded'::character varying, 'failed'::character varying, 'deleted'::character varying])::text[])))
);
...
CREATE TABLE public.suspension_propagation_queue (
    ...
    CONSTRAINT suspension_propagation_queue_action_check CHECK (((action)::text = ANY ((ARRAY['suspend'::character varying, 'restore'::character varying])::text[]))),
    CONSTRAINT suspension_propagation_queue_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'in_progress'::character varying, 'completed'::character varying, 'failed'::character varying])::text[])))
);
...
CREATE TABLE public.tenant_route_policies (
    ...
    CONSTRAINT tenant_route_policies_route_type_check CHECK (((route_type)::text = ANY ((ARRAY['inbound'::character varying, 'outbound'::character varying])::text[]))),
...
CREATE TABLE public.tenant_runtime_policy_events (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT tenant_runtime_policy_events_action_check CHECK (((action)::text = ANY ((ARRAY['activate'::character varying, 'rollback'::character varying])::text[]))),
    CONSTRAINT tenant_runtime_policy_events_stage_check CHECK (((stage)::text = ANY ((ARRAY['precheck'::character varying, 'apply'::character varying, 'verify'::character varying, 'commit'::character varying, 'rollback'::character varying])::text[]))),
    CONSTRAINT tenant_runtime_policy_events_status_check CHECK (((status)::text = ANY ((ARRAY['started'::character varying, 'succeeded'::character varying, 'failed'::character varying])::text[])))
);
...
CREATE TABLE public.tenant_runtime_policy_versions (
    ...
    CONSTRAINT tenant_runtime_policy_versions_build_status_check CHECK (((build_status)::text = ANY ((ARRAY['compiled'::character varying, 'active'::character varying, 'failed'::character varying, 'superseded'::character varying, 'rolled_back'::character varying])::text[]))),
...
CREATE TABLE public.tenant_sip_trunks (
    ...
    CONSTRAINT tenant_sip_trunks_direction_check CHECK (((direction)::text = ANY ((ARRAY['inbound'::character varying, 'outbound'::character varying, 'both'::character varying])::text[]))),
    CONSTRAINT tenant_sip_trunks_port_check CHECK (((port >= 1) AND (port <= 65535))),
    CONSTRAINT tenant_sip_trunks_transport_check CHECK (((transport)::text = ANY ((ARRAY['udp'::character varying, 'tcp'::character varying, 'tls'::character varying])::text[])))
);
...
CREATE TABLE public.tenant_telephony_concurrency_events (
    ...
    created_by uuid,
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT tenant_telephony_concurrency_events_event_type_check CHECK (((event_type)::text = ANY ((ARRAY['acquire'::character varying, 'reject'::character varying, 'release'::character varying, 'expire'::character varying, 'heartbeat'::character varying])::text[]))),
    CONSTRAINT tenant_telephony_concurrency_events_lease_kind_check CHECK (((lease_kind)::text = ANY ((ARRAY['call'::character varying, 'transfer'::character varying])::text[])))
);
...
CREATE TABLE public.tenant_telephony_concurrency_leases (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT chk_tenant_telephony_concurrency_release_consistency CHECK (((((state)::text = ANY ((ARRAY['released'::character varying, 'expired'::character varying])::text[])) AND (released_at IS NOT NULL)) OR ((state)::text = ANY ((ARRAY['active'::character varying, 'releasing'::character varying])::text[])))),
    CONSTRAINT tenant_telephony_concurrency_leases_lease_kind_check CHECK (((lease_kind)::text = ANY ((ARRAY['call'::character varying, 'transfer'::character varying])::text[]))),
    CONSTRAINT tenant_telephony_concurrency_leases_state_check CHECK (((state)::text = ANY ((ARRAY['active'::character varying, 'releasing'::character varying, 'released'::character varying, 'expired'::character varying])::text[])))
);
...
CREATE TABLE public.tenant_telephony_quota_events (
    ...
    CONSTRAINT tenant_telephony_quota_events_event_type_check CHECK (((event_type)::text = ANY ((ARRAY['warn'::character varying, 'throttle'::character varying, 'block'::character varying])::text[]))),
...
CREATE TABLE public.tenant_telephony_threshold_policies (
    ...
    CONSTRAINT tenant_telephony_threshold_policies_policy_scope_check CHECK (((policy_scope)::text = ANY ((ARRAY['api_mutation'::character varying, 'runtime_mutation'::character varying, 'sip_edge'::character varying])::text[]))),
...
CREATE TABLE public.tenant_users (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT tenant_users_status_check CHECK (((status)::text = ANY ((ARRAY['pending'::character varying, 'active'::character varying, 'suspended'::character varying, 'removed'::character varying])::text[])))
);
...
CREATE TABLE public.tenants (
    ...
    CONSTRAINT tenants_status_check CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'suspended'::character varying, 'pending_deletion'::character varying])::text[])))
);
...
CREATE TABLE public.user_profiles (
    ...
    CONSTRAINT chk_user_profiles_role_valid CHECK (((role)::text = ANY ((ARRAY['platform_admin'::character varying, 'partner_admin'::character varying, 'tenant_admin'::character varying, 'user'::character varying, 'readonly'::character varying])::text[])))
);
...
CREATE VIEW public.user_tenant_roles AS
 SELECT up.id AS user_id,
   ...
   FROM (((public.user_profiles up
     ...
     JOIN public.roles r ON ((r.id = tu.role_id)))
  WHERE ((tu.status)::text = ANY ((ARRAY['active'::character varying, 'pending'::character varying])::text[]));
...
CREATE TABLE public.white_label_partners (
    ...
    created_at timestamp with time zone DEFAULT now(),
    updated_at timestamp with time zone DEFAULT now(),
    CONSTRAINT white_label_partners_status_check CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'suspended'::character varying, 'pending_deletion'::character varying])::text[])))
);
...
CREATE UNIQUE INDEX idx_tenant_telephony_concurrency_leases_active_unique ON public.tenant_telephony_concurrency_leases USING btree (tenant_id, call_id, lease_kind) WHERE ((released_at IS NULL) AND ((state)::text = ANY ((ARRAY['active'::character varying, 'releasing'::character varying])::text[])));
...
CREATE INDEX idx_tu_status ON public.tenant_users USING btree (status) WHERE ((status)::text = ANY ((ARRAY['pending'::character varying, 'active'::character varying])::text[]));
...
ALTER TABLE ONLY public.audit_logs
    ADD CONSTRAINT audit_logs_actor_id_fkey FOREIGN KEY (actor_id) REFERENCES public.user_profiles(id);
