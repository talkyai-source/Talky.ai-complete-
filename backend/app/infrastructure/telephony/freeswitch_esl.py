...
from app.domain.services.telephony.transfer_validation import (
    ...
    validate_attended_key,
    validate_esl_command_frame,
...
class TransferRequest:
    ...
    def validate(self) -> None:
        try:
            ...
            self.leg = TransferLeg(self.leg)
        except (TypeError, ValueError) as exc:
            raise ValueError("unsupported transfer mode or leg") from exc
        self.uuid = validate_transfer_call_id(self.uuid)
        self.destination = canonicalize_transfer_destination(
            self.destination,
            self.mode,
        )
        self.context = validate_transfer_context(self.context)
        self.attended_cancel_key = validate_attended_key(
            self.attended_cancel_key,
            "attended_cancel_key",
        )
        self.attended_complete_key = validate_attended_key(
            self.attended_complete_key,
            "attended_complete_key",
        )
        if self.attended_cancel_key == self.attended_complete_key:
            raise ValueError("attended transfer keys must be distinct")
        if (
...
class _ESLConnection:
    ...
    def is_connected(self) -> bool:
        return (
            self._reader is not None and self._writer is not None and not self._writer.is_closing()
        )
    ...
    async def send(self, command: str) -> None:
        ...
        command = validate_esl_command_frame(command)
...
class FreeSwitchESL:
    ...
    def _refresh_connected_state(self) -> None:
        self._connected = self._connection_alive(self._event_conn) and self._connection_alive(
            self._api_conn
        )
    ...
    async def _reconnect_connection(
        ...
        *,
        ...
        async with self._reconnect_lock:
            ...
            if current and (
                failed_conn is None or current is failed_conn or not current.is_connected
            ):
                await current.close()
    ...
    async def _execute_api_command(
        ...
        *,
        ...
        max_attempts = ESL_API_COMMAND_RETRIES + 1 if retry_safe else 1
    ...
    def fence_ownership_loss(self) -> None:
        """Synchronously stop ESL dispatch before transport teardown."""
    ...
    async def _handle_event(self, event: Dict[str, str]) -> None:
        ...
        if event_name and event_name != "HEARTBEAT":
            logger.debug(f"ESL event: {event_name} uuid={call_uuid[:8] if call_uuid else '?'}")
        ...
        if event_name == "CHANNEL_CREATE":
            ...
            logger.info(
                f"📞 New call: {call_info.caller_id} → {call_info.destination} ({call_uuid[:8]})"
            )
            ...
            if app in {"transfer", "uuid_transfer", "att_xfer", "deflect"}:
                if "err" in app_resp or "-err" in app_resp:
                    self._set_transfer_status(
                        attempt_id, TransferStatus.FAILED, "execute_error", app_resp
                    )
                elif "cancel" in app_resp:
                    self._set_transfer_status(
                        attempt_id, TransferStatus.CANCELLED, "execute_cancel", app_resp
                    )
                elif "ok" in app_resp or "+ok" in app_resp:
                    self._set_transfer_status(
                        attempt_id, TransferStatus.SUCCESS, "execute_ok", app_resp
                    )
            return
    ...
    def _build_transfer_command(self, request: TransferRequest) -> str:
        # This method is also called directly by provider adapters and tests;
        # never rely on the HTTP schema or request_transfer() as the sole
    ...
    async def request_transfer(self, request: TransferRequest) -> TransferResult:
        ...
        try:
            if request.mode == TransferMode.DEFLECT and not await self._is_call_answered(
                request.uuid
            ):
                self._set_transfer_status(
                    attempt_id,
            ...
            if request.mode == TransferMode.ATTENDED:
                await self.api(
                    f"uuid_setvar {request.uuid} attxfer_cancel_key {request.attended_cancel_key}"
                )
                await self.api(
                    f"uuid_setvar {request.uuid} attxfer_complete_key {request.attended_complete_key}"
                )

            command = self._build_transfer_command(request)
            # A transport failure after the write is ambiguous. Replaying a
            # mutating transfer can move the call twice, so surface the error
            # and let idempotent domain recovery determine the durable state.
            response = await self.api(command, retry_safe=False)
            result.command = command
            result.response = response

            if "-ERR" in response.upper():
                self._set_transfer_status(
                    attempt_id, TransferStatus.FAILED, "command_rejected", response
                )
                return self._transfer_attempts[attempt_id]

            self._set_transfer_status(
                attempt_id, TransferStatus.ACCEPTED, "command_accepted", response
            )
    ...
    async def api(self, command: str, *, retry_safe: bool = True) -> str:
        """Execute a FreeSWITCH API command and return result."""
        return await self._execute_api_command(
            "api",
            command,
    ...
    async def originate_call(
        self, destination: str, gateway: str = "3cx-pbx", caller_id: str = "1001", timeout: int = 60
    ) -> Optional[str]:
        """
    ...
    async def originate_with_playback(
        self, destination: str, audio_file: str, gateway: str = "3cx-pbx", caller_id: str = "1001"
    ) -> Optional[str]:
