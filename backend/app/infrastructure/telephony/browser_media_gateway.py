...
class BrowserMediaGateway(MediaGateway):
    ...
    def __init__(self):
        ...
        self._target_buffer_ms: int = 100
    ...
    async def initialize(self, config: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"BrowserMediaGateway initialized: {self._sample_rate}Hz, "
    ...
    async def on_audio_received(
        ...
        if not is_valid:
            ...
            if session.input_validation_errors <= 5:
                logger.debug(f"Rejected invalid browser audio chunk for {call_id}: {error}")
            return
        ...
        try:
            if self._tts_source_format == "f32le":
                ...
                if len(audio_chunk) >= 4 and len(audio_chunk) % 4 == 0:
                    float32_arr = np.frombuffer(audio_chunk, dtype=np.float32)
                    max_val = float(np.nanmax(np.abs(float32_arr))) if float32_arr.size else float("inf")
                    if np.isfinite(max_val) and max_val <= 1.5:
                        int16_arr = (np.clip(float32_arr, -1.0, 1.0) * 32767.0).astype(np.int16)
                        audio_chunk = int16_arr.tobytes()
                        logger.debug(f"Converted auto-detected Float32 audio to Int16 (max_val={max_val:.3f})")
    ...
    async def flush_audio_buffer(self, call_id: str) -> None:
        ...
        if session and session.is_active:
            try:
                ...
                if session.pending_byte:
                    logger.debug(
                        "Dropping pending byte to preserve Int16 frame alignment for %s", call_id
                    )
                session.pending_byte = b""
            except Exception as e:
