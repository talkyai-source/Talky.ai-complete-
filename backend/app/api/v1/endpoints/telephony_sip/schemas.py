...
def normalize_trunk_metadata(meta: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    # ``inbound_did`` in trunk metadata was a 2026-08-30 endpoint-set_var
    # workaround. It cannot identify one endpoint when multiple registrations
    # share a carrier source host, and it is not proof that the tenant owns an
    # active DID. The Asterisk reconciler now requires the reviewed carrier
    # inventory to agree with the same-tenant inbound_did_assignments +
    # verified phone-number join.
    if "inbound_did" in out:
        raise ValueError(
            "inbound_did is assignment-owned; configure an inbound DID assignment"
        )
