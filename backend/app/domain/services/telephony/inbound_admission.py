                ...
                bindings = list(
                    await conn.fetch(
                        ...
                        SELECT
                            ...
                            c.knowledge_mode,
                            c.knowledge_model,
                            -- A DID is proven by its verified phone-number
                            -- row. An extension has none: it is proven by the
                            -- trunk that registers it, which the address
                            -- predicate below pins with st.auth_username.
                            COALESCE(pn.status, 'verified') AS phone_status,
                        ...
                        WHERE (
                             ...
                             OR (a.extension IS NOT NULL
                                 AND 'ext:' || a.extension = $1
                                 AND st.auth_username = a.extension)
                          )
                          AND a.status='active'
                          AND a.valid_from <= NOW()
                          AND (a.valid_to IS NULL OR a.valid_to > NOW())
